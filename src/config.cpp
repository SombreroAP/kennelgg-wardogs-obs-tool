...
void Config::load()
{
	...
	DEFS(gameSource); DEFS(sceneName); DEFI(activeFriend); DEFB(bringToFront); DEFB(keepWarm);
	DEFB(lookName); DEFB(lookPlate); DEFB(lookCam); DEFB(lookGrain); DEFB(lookVignette); DEFS(lookLabel); DEFI(grainAmount);
	DEFB(autoDetect); DEFB(enabled); DEFD(threshold); DEFI(pollMs); DEFI(downFrames); DEFI(upFrames); DEFI(minDownMs);
	DEFB(watchRevive); DEFD(reviveThreshold); DEFD(customTemplateWidthFrac); DEFD(boxX); DEFD(boxY); DEFD(boxW); DEFD(boxH);

	GETS(gameSource); GETS(sceneName); GETI(activeFriend); GETB(bringToFront); GETB(keepWarm);
	GETB(lookName); GETB(lookPlate); GETB(lookCam); GETB(lookGrain); GETB(lookVignette); GETS(lookLabel); GETI(grainAmount);
	GETB(autoDetect); GETB(enabled); GETD(threshold); GETI(pollMs); GETI(downFrames); GETI(upFrames); GETI(minDownMs);
	GETB(watchRevive); GETD(reviveThreshold); GETD(customTemplateWidthFrac); GETD(boxX); GETD(boxY); GETD(boxW); GETD(boxH);
...
void Config::save() const
{
	...
	SETS(gameSource); SETS(sceneName); SETI(activeFriend); SETB(bringToFront); SETB(keepWarm);
	SETB(lookName); SETB(lookPlate); SETB(lookCam); SETB(lookGrain); SETB(lookVignette); SETS(lookLabel); SETI(grainAmount);
	SETB(autoDetect); SETB(enabled); SETD(threshold); SETI(pollMs); SETI(downFrames); SETI(upFrames); SETI(minDownMs);
	SETB(watchRevive); SETD(reviveThreshold); SETD(customTemplateWidthFrac); SETD(boxX); SETD(boxY); SETD(boxW); SETD(boxH);
